...
export default async function CaseDetailPage({
  ...
  const recSummary = (recommendation?.summary ?? recommendation?.diagnosis ?? caseRecord.description ?? "No recommendation summary available.") as string;
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between rounded-xl border border-amber-500/20 bg-amber-500/5 p-4 gap-3">
                <div>
                  ...
                  <p className="text-3xl font-extrabold text-foreground tabular-nums">
                    {formatCurrency(caseRecord.amountAtRisk, caseRecord.currency ?? "INR")}
                  </p>
                </div>
                <div className="flex flex-col items-start sm:items-end gap-2 text-xs text-muted-foreground">
                  <div>
                    ...
                    <span className={caseRecord.requiresHumanApproval ? "text-amber-600 dark:text-amber-400 font-semibold" : "text-emerald-600 dark:text-emerald-400 font-semibold"}>
                      {caseRecord.requiresHumanApproval ? "Human Approval Required" : "Automated Recovery Eligible"}
                    </span>
                  </div>
                </div>
              </div>
            ...
            <CardContent className="p-6 space-y-4 text-sm flex-1">
              <div className="space-y-1">
                ...
                <p className="font-mono text-xs font-bold text-foreground bg-muted/40 p-2 rounded-md border">
                  {caseRecord.payment?.razorpayPaymentId ?? "Not linked"}
                </p>
              </div>
              ...
              <div className="space-y-1">
                ...
                <p className="font-mono text-xs font-bold text-foreground bg-muted/40 p-2 rounded-md border">
                  {caseRecord.order?.razorpayOrderId ?? "Not linked"}
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 pt-2">
                <div>
                  ...
                  <Badge variant="outline" className="font-mono text-xs mt-1 capitalize">
                    {(caseRecord.payment?.method ?? "Unknown").toLowerCase()}
                  </Badge>
                </div>
                <div>
          ...
          <Card className="border border-border/80 shadow-sm">
            ...
            <CardContent className="p-5 space-y-3 text-xs">
              <p className="text-sm font-medium text-foreground leading-relaxed">
                {String(analysis?.diagnosis ?? caseRecord.description ?? "No diagnosis available")}
              </p>
              <div className="flex items-center justify-between border-t pt-2 text-muted-foreground">
                <span>Confidence:</span>
                <Badge variant="outline" className="font-mono font-semibold">
                  {String(analysis?.confidence ?? "N/A")}
                </Badge>
              </div>
          ...
          <Card className={caseRecord.status === "PENDING_APPROVAL" ? "border border-amber-500/50 shadow-sm bg-amber-500/5" : "border border-border/80 shadow-sm"}>
            <CardHeader className="border-b bg-muted/20 px-5 py-3">
              <CardTitle className="text-sm font-bold flex items-center gap-2">
                ...
                {caseRecord.status === "PENDING_APPROVAL" ? "Awaiting Human Approval" : "Execution Status"}
              </CardTitle>
            </CardHeader>
            <CardContent className="p-5 space-y-3 text-xs">
        ...
        <Card className="border border-border/80 shadow-sm overflow-hidden">
          ...
          <CardContent className="p-0">
            {evidence.length > 0 ? (
              ...
              <p className="p-6 text-xs text-muted-foreground">No structured evidence signals stored for this case.</p>
            )}
          </CardContent>
        </Card>
          ...
          <Card className="border border-border/80 shadow-sm flex flex-col">
            ...
            <CardContent className="p-6 space-y-4 flex-1 overflow-y-auto max-h-[420px]">
              {caseRecord.agentRuns.length === 0 ? (
                <p className="text-xs text-muted-foreground">No agent execution runs recorded.</p>
              ) : (
